...
class _UnifiedCore:
    def __init__(self, N, periods, E=1):
        ...
        self.ent_buf = np.zeros((0,), dtype=int)              # entity id of buf row
    ...
    def _factor_update(self, resid, obs, z_prev, irls=True):
        ...
        rhs = WtP @ ro + m0 / s
        try:
            c = cho_factor(prec, lower=True, check_finite=False)
            z_post = cho_solve(c, rhs, check_finite=False)
        except Exception:
    ...
    def process_row(self, x_obs_row, abs_t, z_prev, fourier_t, eid=0):
        ...
        self.ent_buf = np.append(self.ent_buf, eid)
        if self.res_buf.shape[0] > WINDOW:
            ...
            self.ent_buf = self.ent_buf[-WINDOW:]

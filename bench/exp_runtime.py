...
def make_figure(sc):
    ...
    axT.set_title(f"CAFE vs $T$  (fixed $N={sc['N_fix']}$)", fontsize=9)
    ...
    axN.set_title(f"CAFE vs $N$  (fixed $T={sc['T_fix']}$)", fontsize=9)
...
def make_table(rows, task):
    ...
    lines.append("\\toprule")
    lines.append("Method & Backbone & Wall-clock (s)~$\\downarrow$ & MAE~$\\downarrow$ "
                 "& Causal? & HW \\\\")

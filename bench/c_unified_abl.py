...
# --------------------------------------------------------------------------- #
# ABLATION SWITCHES (scratch file only -- NOT in the production core).
# Each toggle disables exactly one of CAFE's four learned dials so the paper's
# ablation can show each one earns its place. With all flags False the model is
# bit-for-bit identical to c_unified_penmf.online_impute.
#   no_robust : force Gaussian -> nu pinned at +inf so IRLS weight w(r)=1 and the
#               redescending winsorize band never clips.
#   no_ar     : force a=0 -> static factors, no AR carry / no idiosyncratic carry.
#   no_ard    : disable ARD pruning -> alpha / s_fac / beta_alpha held constant
#               (full constant rank), so the data cannot shrink unused factors.
#   no_season : zero the Fourier design -> beta=0 always (no seasonal term).
# --------------------------------------------------------------------------- #
_NU_INF = 1.0e12        # "Gaussian" dof: w=(nu+1)/(nu+r2/s2) -> 1 for any finite r2

_ABL = {"no_robust": False, "no_ar": False, "no_ard": False, "no_season": False}


def set_ablation(**flags):
    """Set the active ablation flags (module-global, read by _UnifiedCore)."""
    for k in flags:
        if k not in _ABL:
            raise KeyError(f"unknown ablation flag {k!r}")
    _ABL.update(flags)
...
class _UnifiedCore:
    def __init__(self, N, periods, E=1):
        # snapshot the active ablation flags onto the instance
        self.abl = dict(_ABL)
        if self.abl["no_season"]:
            periods = []                      # zero the Fourier design entirely
        self.N = N
        self.E = E
        ...
        self.t_seen = 0.0
        self.a = 0.0 if self.abl["no_ar"] else 0.5           # AR coeff on z (learned)
        # no_robust: pin nu at +inf so the IRLS weight is identically 1 (Gaussian).
        self.nu = _NU_INF if self.abl["no_robust"] else NU_INIT  # Student-t dof
    ...
    def _refit_W(self):
        ...
        # no_ard: hold alpha / s_fac CONSTANT (unit) -> no data-driven pruning; the
        # full R_MAX rank is always offered with a fixed ridge, so the data can never
        # shrink unused factors (the effective rank can no longer emerge).
        if not self.abl["no_ard"]:
        ...
        if A.shape[0] >= 2 and not self.abl["no_ard"]:
        ...
        # no_ar: a is pinned at 0 (static factors) and never re-estimated.
        if A.shape[0] >= 2 and not self.abl["no_ar"]:
            zt = A[1:]
            ztm = A[:-1]
            if ent is not None:
                pair = ent[1:] == ent[:-1]
    ...
    def _update_robust_scale(self, r2_vals):
        ...
        if self.mom_cnt > 30.0 and not self.abl["no_robust"]:
    ...
    def process_row(self, x_obs_row, abs_t, z_prev, fourier_t, eid=0):
        ...
        if obs.any():
            ...
            # no_ar disables ALL temporal carry: the idiosyncratic AR(1) rho is pinned
            # at 0 too (no carry term survives), consistent with a=0 static factors.
            if np.any(fresh) and not self.abl["no_ar"]:
                pn = prev[fresh]
...
def _impute_panel(X, meta):
    ...
    abl = dict(_ABL)                                 # ablation flags for the panel path
    rng = np.random.default_rng(0)
    ...
    a_ar = 0.0 if abl["no_ar"] else 0.5              # learned AR coeff on time factor
    g_prev = np.zeros(R_MAX)
    nu = _NU_INF if abl["no_robust"] else NU_INIT; scale2 = 1.0
    ...
    for t in uniq_t:
        ...
        if rv:
            ...
            if mc > 30.0 and not abl["no_robust"]:

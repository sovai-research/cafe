...
$0.19$ (TRMF)---indeed TRMF's honest \emph{live} $R^2$ ($0.16$) is \emph{worse}
than \cafe{}'s ($0.26$) despite a better-looking reported number---while
\cafe{}'s reported and live scores are identical by construction
...
tails on heavy-tailed data (MAE $1.33\!\to\!2.08$ when removed), the AR
coefficient on dynamic block-gap data, and ARD rank on drifting data. The
seasonal dial is the exception---it is decisive on a strongly periodic
\emph{univariate} series (Seas-1D, $0.58\!\to\!1.37$ if removed) but roughly
neutral on multivariate panels, where the low-rank term already absorbs periodic
structure and ARD shrinks the harmonics toward zero. We keep it because it is
ARD-gated and never materially hurts, but it is the one component that is
...
$1.4$\,s on a single CPU core---the lowest MAE of any baseline, at cost
comparable to SoftImpute and below TRMF (Table~\ref{tab:runtime}). Cost is
linear in series length $T$ (measured log-log slope $1.03$) and sub-linear in
width $N$ (slope $0.25$): each row performs only rank-$R$ Cholesky solves, never
...
length $T$ (log-log slope $1.03$, near-linear) and vs.\ width $N$ (slope $0.25$,

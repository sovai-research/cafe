...
\emph{bidirectional} (use the future). Linear interpolation (our run) is the
telling cheap baseline: under \emph{scattered} point missingness a gap is flanked
by observed neighbours, so a straight line between them is hard to beat---it
already edges every GPU deep net---but it too is \emph{non-causal} (it reads the
next observed value). \cafe{} is the only causal method here, and still lowest, on
one CPU core.}
\label{tab:sota}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Linear interp             & .135 & \ding{55} & interp & CPU \\
\textbf{\cafe{} (ours)}   & $\mathbf{.108}$ & \ding{51} & factor & \textbf{CPU} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Uncertainty.} The same pass returns a per-cell posterior interval. It
is \emph{conservative by construction}: across datasets the empirical coverage
never falls below nominal (Table~\ref{tab:calib}), so a band is never falsely
tight---the safe failure mode for risk-sensitive use. The per-cell $\sigma$
widens correctly inside long gaps (Fig.~\ref{fig:unc}) and is informative (lower
CRPS than a homoscedastic $\sigma$) on the high-frequency ETTh1 data; tightening
its absolute scale on smoother panels is future work. This is the one capability
we evaluate quantitatively beyond imputation---the remaining outputs (factors,
anomaly score, decomposition, dependency network, forecasts) are exact readouts
of the same fit, illustrated in App.~\ref{sec:appendix} rather than benchmarked
against specialised tools.
...
\caption{\textbf{Calibration (Table~\ref{tab:calib}).} \emph{(a)} Reliability
diagram: empirical vs.\ nominal coverage---every curve sits \emph{on or above} the
diagonal, so \cafe{} never under-covers. \emph{(b)} PIT histograms: a flat
profile would be sharply calibrated; \cafe{}'s are centre-peaked, the signature of
conservative (over-dispersed) intervals---safe, but loose, on the smoother panels.}
\label{fig:calib}
\end{figure}

...
that subsumes the zoo: a robust dynamic factor model with two-way fixed effects,
optimised online by one penalised objective whose continuous parameters
...
quality, $132$ series). \cafe{} is CPU-only, requires no configuration, and runs
the full benchmark suite in $\sim\!1$\,s.
\end{abstract}
...
(block missingness) and point-missing protocols. Among strictly \emph{causal}
